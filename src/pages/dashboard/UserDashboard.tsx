...
import {
  ...
  CheckCircle2, XCircle, AlertCircle, Wallet, Info
} from "lucide-react";
...
const allSections = [
  ...
  { icon: Truck, label: "الطلبات", labelEn: "Orders", key: "orders" },
...
const UserDashboard = () => {
  ...
  const recordAttendance = async (type: "in" | "out") => {
    ...
    if (type === "in") {
      if (hour < 8 || (hour === 8 && minute <= 15)) {
        status = t("في الوقت ✅", "On time ✅");
      } else if (hour === 8 && minute > 15 || hour === 9) {
        status = t("متأخر ⚠️", "Late ⚠️");
        deduction = 10;
      } else {
        status = t("غياب جزئي 🔴", "Partial absence 🔴");
        deduction = 30;
      }
    } else {
      if (hour < 16) {
        status = t("خروج مبكر بدون إذن ⛔", "Early leave without permission ⛔");
        deduction = 20;
      } else {
        status = t("انصراف طبيعي ✅", "Normal checkout ✅");
      }
    }

    const confirmMsg = type === "in" 
      ? t(`سيتم تسجيل حضورك الآن. حالتك: ${status}${deduction > 0 ? ` - سيتم خصم ${deduction} د.ل من راتبك` : ""}`, `Recording check-in. Status: ${status}`)
      : t(`سيتم تسجيل انصرافك الآن. حالتك: ${status}${deduction > 0 ? ` - سيتم خصم ${deduction} د.ل من راتبك` : ""}`, `Recording check-out. Status: ${status}`);
  ...
  const submitRequest = async (type: string, data: any) => {
    await supabase.from("employee_requests").insert({
      company_id: companyId!, employee_id: myData?.id, type,
      reason: data.reason || "", amount: Number(data.amount) || 0,
      start_date: data.from || "", end_date: data.to || "",
    });
    ...
    <div className="min-h-screen flex bg-background">
      <aside className={`fixed inset-y-0 ${lang === "ar" ? "right-0 border-l" : "left-0 border-r"} w-64 bg-card border-border z-50 transform transition-transform md:translate-x-0 ${sidebarOpen ? "translate-x-0" : lang === "ar" ? "translate-x-full md:translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        ...
        <div className="absolute bottom-0 right-0 left-0 p-3 border-t border-border">
          <button onClick={logout} className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-destructive hover:bg-destructive/10">
        ...
        <div className="p-4 md:p-6 space-y-0">
          ...
          {!permissions.includes(activeTab) && !["dashboard", "my-info", "attendance", "requests", "my-tasks"].includes(activeTab) && (
            <div className={`${cardClass} text-center py-12`}>
              ...
              <p className="text-sm text-muted-foreground">{t("ليس لديك صلاحية للوصول إلى هذا القسم. تواصل مع مسؤول الشركة لتفعيل هذه الصلاحية.", "No permission. Contact your company admin.")}</p>
            </div>
          )}
          ...
          {activeTab === "my-info" && (
            <div className="space-y-4">
              <div className={cardClass}>
                ...
                <p className="text-xs text-muted-foreground mb-4">{t("جميع بياناتك الوظيفية كما حددها مسؤول الشركة. للتعديل، تواصل مع مسؤولك.", "Your employment details as set by company admin.")}</p>
              ...
              <div className={cardClass}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="glass rounded-xl p-3 text-center"><p className="text-[10px] text-muted-foreground">{t("الخصومات","Deductions")}</p><p className="text-lg font-black text-destructive">{attendance.reduce((a, r) => a + (r.deduction || 0), 0)}</p></div>
                  ...
                  <div className="glass rounded-xl p-3 text-center"><p className="text-[10px] text-muted-foreground">{t("الصافي","Net")}</p><p className="text-lg font-black text-primary">{baseSalary - attendance.reduce((a, r) => a + (r.deduction || 0), 0)}</p></div>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === "attendance" && (
            <div className="space-y-4">
              <div className={cardClass}>
                ...
                <p className="text-xs text-muted-foreground mb-4">{t("سجّل حضورك وانصرافك يومياً. سيتم احتساب التأخير والغياب والخصومات تلقائياً.", "Record daily check-in/out. Late, absence, and deductions are automatic.")}</p>
                ...
                <div className="bg-primary/10 border border-primary/30 rounded-xl p-4 mb-4">
                  ...
                  <p className="text-xs text-warning mt-1">{t("⚠️ التأخير: خصم 10 د.ل · الغياب الجزئي: 30 د.ل · الخروج المبكر بدون إذن: 20 د.ل","⚠️ Late: -10 LYD · Partial absence: -30 LYD · Early leave: -20 LYD")}</p>
                </div>

                <div className="flex gap-3 mb-6">
                  {(!todayRecord || !todayRecord.check_in) && (
                    <button onClick={() => recordAttendance("in")} className={`${btnPrimary} flex items-center gap-2`}>
                  ...
                  {todayRecord?.check_in && todayRecord?.check_out && (
                    <div className="glass rounded-xl p-4 border-success/30 w-full">
                      <p className="text-sm text-success font-bold">✅ {t("تم تسجيل حضورك وانصرافك اليوم بنجاح", "Today's attendance complete")}</p>
                      <p className="text-xs text-muted-foreground mt-1">{t("حضور:","In:")} {todayRecord.check_in} · {t("انصراف:","Out:")} {todayRecord.check_out}</p>
                    </div>
                  )}
                </div>

                <h4 className="font-bold text-foreground mb-3">{t("سجل الحضور", "Attendance Log")}</h4>
                {attendance.length === 0 ? <p className="text-sm text-muted-foreground">{t("لا يوجد سجل حضور.", "No records.")}</p> : (
                  <div className="space-y-2">{attendance.map(a => (
                    <div key={a.id} className="glass rounded-xl p-3 flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        {a.deduction > 0 && <p className="text-[10px] text-destructive font-bold">{t("خصم:","Deduction:")} {a.deduction} {t("د.ل","LYD")}</p>}
                      </div>
                    </div>
                  ))}</div>
                )}
          ...
          {activeTab === "requests" && (
            <div className="space-y-4">
              <div className={cardClass}>
                ...
                <p className="text-xs text-muted-foreground mb-4">{t("أرسل طلبات الإجازة أو السلفة أو سحب الراتب المبكر. ستراجعها إدارة الشركة.", "Submit leave, advance, or salary withdrawal requests.")}</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="glass rounded-xl p-4">
                    ...
                    <form onSubmit={e => { e.preventDefault(); const fd = new FormData(e.target as HTMLFormElement); submitRequest("leave", Object.fromEntries(fd)); (e.target as HTMLFormElement).reset(); }} className="space-y-2">
                      <select name="leaveType" className={inputClass}><option>{t("سنوية", "Annual")}</option><option>{t("مرضية", "Sick")}</option><option>{t("طارئة", "Emergency")}</option><option>{t("بدون راتب","Unpaid")}</option></select>
                  ...
                  <div className="glass rounded-xl p-4">
                    ...
                    <form onSubmit={e => { e.preventDefault(); const fd = new FormData(e.target as HTMLFormElement); submitRequest("advance", Object.fromEntries(fd)); (e.target as HTMLFormElement).reset(); }} className="space-y-2">
                      <input name="amount" type="number" required className={inputClass} placeholder={t("المبلغ المطلوب", "Amount")} />
                      <textarea name="reason" rows={2} className={inputClass} placeholder={t("سبب الطلب", "Reason")} />
                      <button type="submit" className={`w-full ${btnPrimary} flex items-center justify-center gap-2`}><Send className="h-4 w-4" /> {t("إرسال", "Submit")}</button>
                    </form>
                  </div>
                  <div className="glass rounded-xl p-4">
                ...
                {myRequests.length === 0 ? <p className="text-sm text-muted-foreground">{t("لا توجد طلبات سابقة.", "No previous requests.")}</p> : (
                  <div className="space-y-2">{myRequests.map(r => (
                    <div key={r.id} className="glass rounded-xl p-3 flex items-center justify-between">
                      <div>
                        <p className="text-sm font-bold text-foreground">{r.type === "leave" ? t("إجازة","Leave") : r.type === "advance" ? t("سلفة","Advance") : t("سحب راتب","Salary")} {r.reason ? `- ${r.reason}` : ""}</p>
                        <p className="text-xs text-muted-foreground">{new Date(r.created_at).toLocaleDateString("ar-LY")} {r.amount > 0 ? `· ${r.amount} ${t("د.ل","LYD")}` : ""}</p>
                        {r.admin_notes && <p className="text-[10px] text-primary mt-1">{t("ملاحظات الإدارة:","Admin notes:")} {r.admin_notes}</p>}
                      </div>
          ...
          {activeTab === "my-tasks" && (
            <div className="space-y-4">
              <div className={cardClass}>
                ...
                <p className="text-xs text-muted-foreground mb-4">{t("المهام المسندة إليك من مسؤول الشركة. أكملها في الوقت المحدد.", "Tasks assigned by company admin. Complete them on time.")}</p>
                {tasks.length === 0 ? (
                  <div className="text-center py-8"><ListChecks className="h-12 w-12 text-muted-foreground mx-auto mb-3" /><p className="text-sm text-muted-foreground">{t("لا توجد مهام مسندة إليك حالياً.", "No tasks assigned.")}</p></div>
                ) : (
                  <div className="space-y-3">{tasks.map(tk => (
                    <div key={tk.id} className={`glass rounded-xl p-4 ${tk.priority === "high" ? "border-l-4 border-l-destructive" : tk.priority === "medium" ? "border-l-4 border-l-warning" : "border-l-4 border-l-success"}`}>
                      ...
                      <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                        {tk.due_date && <span>📅 {t("التسليم:","Due:")} {tk.due_date}</span>}
                        <span>🔥 {tk.priority === "high" ? t("عالية","High") : tk.priority === "medium" ? t("متوسطة","Medium") : t("منخفضة","Low")}</span>
                      </div>
                      {tk.status === "pending" && (
                        <button onClick={async () => { await supabase.from("tasks").update({ status: "completed" }).eq("id", tk.id); setTasks(tasks.map(t => t.id === tk.id ? {...t, status: "completed"} : t)); }} className="mt-2 px-3 py-1 rounded text-xs bg-success/20 text-success font-bold">{t("تم الإنجاز","Complete")}</button>
                      )}
                    </div>
                  ))}</div>
                )}
          ...
          {/* ======= PRODUCTS (read-only for employees) ======= */}
          {activeTab === "products" && permissions.includes("products") && (
            <div className="space-y-4">
              <div className={cardClass}>
                <h3 className="font-bold text-foreground mb-2">{t("المنتجات","Products")} ({products.length})</h3>
                <p className="text-xs text-muted-foreground mb-4">{t("قائمة المنتجات المتاحة في مخازن الشركة.","Available products in company warehouses.")}</p>
                {products.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm"><thead><tr className="border-b border-border">{[t("الاسم","Name"),t("الكود","Code"),t("النوع","Type"),t("الكمية","Qty"),t("بيع","Sell")].map(h => <th key={h} className="text-right py-2 px-2 text-muted-foreground text-xs">{h}</th>)}</tr></thead>
                      <tbody>{products.map(p => (<tr key={p.id} className="border-b border-border/30"><td className="py-2 px-2 text-xs font-bold">{p.name}</td><td className="py-2 px-2 text-xs text-muted-foreground">{p.code||"-"}</td><td className="py-2 px-2 text-xs">{p.type||"-"}</td><td className="py-2 px-2 text-xs font-bold">{p.quantity}</td><td className="py-2 px-2 text-xs text-primary font-bold">{p.sell_price}</td></tr>))}</tbody>
                    </table>
                  </div>
          ...
          {activeTab === "stock" && permissions.includes("stock") && (
            <div className="space-y-4">
              <div className={cardClass}>
                <h3 className="font-bold text-foreground mb-4">{t("حركة المخزون","Stock Movements")}</h3>
                {movements.length > 0 ? (
            ...
            <div className="space-y-4">
              <div className={cardClass}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="glass rounded-xl p-3 text-center"><p className="text-xs text-muted-foreground">{t("الأرباح","Profits")}</p><p className="text-lg font-black text-success">{Math.max(0, products.reduce((a,p) => a + ((p.sell_price||0)-(p.buy_price||0))*(p.quantity||0),0)).toLocaleString()}</p></div>
                  <div className="glass rounded-xl p-3 text-center"><p className="text-xs text-muted-foreground">{t("الطلبات","Orders")}</p><p className="text-lg font-black">{orders.length}</p></div>
                </div>
              </div>
            </div>
          )}

          {/* Generic for other permission-based tabs */}
          {["invoices","reports","hr","users","settings","orders","suppliers","barcode","returns","inventory"].includes(activeTab) && permissions.includes(activeTab) && !["products","stock","accounting"].includes(activeTab) && (
            <div className={`${cardClass} text-center py-12`}>
              <Info className="h-12 w-12 text-primary mx-auto mb-3" />
              <h3 className="font-bold text-foreground">{visibleSections.find(s=>s.key===activeTab)?.label}</h3>
              <p className="text-sm text-muted-foreground mt-2">{t("هذا القسم متاح لك حسب صلاحياتك. يمكنك عرض البيانات والتقارير.","This section is available based on your permissions.")}</p>
            </div>
          )}
        </div>
      </main>
